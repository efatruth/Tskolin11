...
from math import *
def writeFile(nameFile,mode):
    #Búið til textaskrá...
    print("in file")
    with open(nameFile,mode,encoding='utf-8') as f: #býr til skjalið tolur.txt og gerir það mögulegt að edita í það
...
writeFile("tolur.txt","w")
#a

print("\nLiður a:")
def readfile(filename,mode):
    with open(filename,mode,encoding='utf-8') as f: #opnar skjalið tolur.txt og gerir það mögulegt að nota innihald þess
        ...
        tolulisti.remove('') #eyðir tómu value-i úr tolulist
    return tolulisti

print(readfile("tolur.txt","r"))
